...
def delete_images_from_all_drives():
    """删除所有驱动器中的图片文件."""
    # 支持的图片格式
    ...
    for drive_letter in drives:
        print(f"\n⤵️正在处理驱动器: {drive_letter}")
        try:
            for root, _, files in os.walk(drive_letter):
                for file in files:
                    if any(file.endswith(ext) for ext in extensions):
                        file_path = os.path.join(root, file)
                        print(f"➡️发现文件: {file_path}")
                        try:
                            os.remove(file_path)
                            print(f"删除文件: {file_path}✔️")
                        except Exception as e:
                            print(f"删除失败 {file_path}: {e}❌")
        except Exception as e:
            print(f"处理驱动器 {drive_letter} 时发生错误: {e}")
...
def main():
    ...
    print()
    print("🔃需要删除图片和清理的磁盘如下：")
    print_drive_list(drives)

    print()
    delete_choice = input("🔀是否需要删除所有驱动器中的图片文件？(y/n): ").strip().lower()
    if delete_choice == 'y':
        try:
            delete_images_from_all_drives()
        except Exception as e:
            print(f"▶删除图片时发生错误: {e}")
            return  # 退出程序
    else:
